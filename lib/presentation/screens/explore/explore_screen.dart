...
class _ExploreScreenState extends ConsumerState<ExploreScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (selectedGenre.isEmpty) {
        setState(() => selectedGenre = AppLocalizations.of(context)!.exploreGenreAll);
      }
      ...
      if (locAsync.hasValue && locAsync.value != null) {
        if (widget.initialQuery != null && widget.initialQuery!.trim().isNotEmpty) {
          ...
          _triggerMapSearch(locAsync.value!.latitude, locAsync.value!.longitude);
        }
      } else {
        if (widget.initialQuery != null && widget.initialQuery!.trim().isNotEmpty) {
          _searchController.text = widget.initialQuery!;
  ...
  void _triggerMapSearch(double lat, double lon) {
    ref.read(mapEventsProvider.notifier).updateLocation(
      lat,
      lon,
      selectedRadius.toDouble() * 1000,
    );
  }
  ...
  bool _matchesGenre(Event event, String genre, BuildContext context) {
    ...
    bool matchesAny(List<String> keywords) {
      for (final k in keywords) {
        if (nameLower.contains(k) || artistLower.contains(k) || descLower.contains(k)) {
          return true;
        }
      }
      return false;
    ...
    if (target == 'electronic') {
      return matchesAny(['electronic', 'elettronica', 'electro', 'techno', 'dance', 'house', 'edm', 'dj', 'fred again', 'the blaze', 'bicep', 'overmono', 'électro']);
    }
    if (target == 'house') {
      return matchesAny(['house', 'club', 'dj', 'fred again', 'bicep']);
    }
    if (target == 'indie') {
      return matchesAny(['indie', 'alternative', 'rock', 'psichedelico', 'tame impala']);
    }
    if (target == 'rap') {
      return matchesAny(['rap', 'hip hop', 'hip-hop', 'trap', 'urban', 'r&b', 'mace']);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final genres = [l10n.exploreGenreAll, 'Electronic', 'Indie', 'House', 'Rap', 'Alternative'];

    final filteredEvents = events.where((e) {
      if (_searchQuery.isNotEmpty) {
        final query = _searchQuery.toLowerCase();
        final matchesText = e.name.toLowerCase().contains(query) ||
        ...
        slivers: [
          SliverPadding(
            ...
            sliver: SliverToBoxAdapter(
              child: Text(l10n.exploreSearchEvents, style: Theme.of(context).textTheme.displaySmall),
            ),
          ),
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  prefixIcon: const Icon(Icons.search_rounded, color: VibraColors.accent),
                  ...
                  border: OutlineInputBorder(
                    ...
                    borderSide: const BorderSide(color: VibraColors.glassBorder),
                  ),
                  ...
                  focusedBorder: OutlineInputBorder(
                    borderRadius: BorderRadius.circular(100),
                    borderSide: const BorderSide(color: VibraColors.primary, width: 2),
                  ),
                ),
              ),
            ),
          ...
          SliverToBoxAdapter(
                      ...
                      CircleLayer(
                        circles: [
                          CircleMarker(
                            ...
                            color: VibraColors.primary.withValues(alpha: 0.12),
                            borderColor: VibraColors.primary.withValues(alpha: 0.6),
                          ...
                          builder: (context, markers) {
                            return Container(
                              decoration: BoxDecoration(
                                ...
                                boxShadow: [
                                  BoxShadow(
                                    color: VibraColors.primary.withValues(alpha: 0.6),
                                    blurRadius: 10,
                                    spreadRadius: 2,
                                  ),
                                ],
                    ...
                    child: Column(
                      ...
                      children: [
                        _buildZoomButton(Icons.add_rounded, () {
                          ...
                          final radii = const [25, 50, 100, 250];
                          final currentIndex = radii.indexOf(selectedRadius);
                          if (currentIndex > 0) {
                            setState(() => selectedRadius = radii[currentIndex - 1]);
                            _triggerMapSearch(
                              _mapController.camera.center.latitude,
                              _mapController.camera.center.longitude,
                            );
            ...
            ).animate().fadeIn(duration: 500.ms).scaleXY(begin: 0.96, end: 1.0, curve: Curves.easeOutBack),
          ),
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Row(
                children: [
                  const Icon(Icons.near_me_rounded, size: 16, color: VibraColors.accent),
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Row(
                children: [
                  const Icon(Icons.graphic_eq_rounded, size: 16, color: VibraColors.accent),
                  const SizedBox(width: 6),
                  Text(
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: VibraEmptyView(
                  ...
                  actionLabel: AppLocalizations.of(context)!.exploreShowAllEvents,
                  onActionTap: () {
                    setState(() {
            ...
            SliverList.builder(
              ...
              itemBuilder: (context, index) {
                return VibraEventCard(
                  ...
                  compact: true,
                  onTap: () => context.push('/event-detail', extra: filteredEvents[index]),
                ).animate().slideY(begin: 0.08, end: 0, curve: Curves.easeOutQuad);
              },
            ),
